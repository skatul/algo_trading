...
class MovingAverageCrossoverStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        buy_condition = (df['position_change'] == 1) & (~df[short_ma_col].isna()) & (~df[long_ma_col].isna())
        df.loc[buy_condition, 'signal'] = 1
        df.loc[buy_condition, 'signal_reason'] = 'MA_BULLISH_CROSS'
        
        # Sell signal when position changes from 1 to 0
        sell_condition = (df['position_change'] == -1) & (~df[short_ma_col].isna()) & (~df[long_ma_col].isna())
