...
class BacktestEngine:
    ...
    def run_backtest(self, 
        ...
        self.logger.info(f"Backtest completed. Final portfolio value: ${self.results['final_capital']:.2f}")
    ...
    def _simulate_trading(self, signal_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(df)):
            ...
            if i > 0:
                ...
                if prev_value > 0:
                    df.loc[current_date, 'returns'] = (portfolio_value - prev_value) / prev_value
    ...
    def plot_results(self, save_path: Optional[str] = None):
        ...
        # Portfolio value
        axes[0].plot(data.index, data['portfolio_value'], label='Portfolio Value', color='blue')
        ...
        # Returns
        axes[1].plot(data.index, data['returns'], label='Daily Returns', color='green', alpha=0.7)
        ...
        axes[2].fill_between(data.index, drawdown, 0, color='red', alpha=0.3, label='Drawdown')
    ...
    def print_summary(self):
        ...
        print(f"{'='*60}")
        print(f"Period: {self.results['start_date'].date()} to {self.results['end_date'].date()}")
...
def compare_strategies(strategies: List[Dict], 
                      data: pd.DataFrame,
