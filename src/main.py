...
class TradingEngine:
    ...
    def create_strategy(self, 
        ...
        self.logger.info(f"Created strategy: {strategy.name} with parameters: {parameters}")
        return strategy
    ...
    def backtest(self,
        ...
        if self.current_data.empty or symbol not in self.current_data.get('symbol', pd.Series()).values:
    ...
    def compare_strategies_on_symbol(self, 
        ...
        # Run comparison
        initial_capital = kwargs.get('initial_capital') or self.config.get('trading.initial_capital', 100000)
        comparison_results = compare_strategies(strategy_list, self.current_data, initial_capital)
        ...
        for strat_name in strategies:
            if strat_name == 'moving_average':
                strategy_configs.extend([
                    {'name': 'MA_10_30', 'strategy': 'moving_average', 'parameters': {'short_window': 10, 'long_window': 30}},
                    {'name': 'MA_20_50', 'strategy': 'moving_average', 'parameters': {'short_window': 20, 'long_window': 50}}
                ])
            elif strat_name == 'rsi':
                strategy_configs.extend([
                    {'name': 'RSI_14', 'strategy': 'rsi', 'parameters': {'rsi_period': 14}},
                    {'name': 'RSI_21', 'strategy': 'rsi', 'parameters': {'rsi_period': 21}}
                ])
            elif strat_name == 'buy_and_hold':
                strategy_configs.append({'name': 'Buy_Hold', 'strategy': 'buy_and_hold', 'parameters': {}})
