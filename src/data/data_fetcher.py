...
class DataFetcher:
    ...
    def get_yahoo_data(
        ...
        try:
            ...
            self.logger.info(
                "Successfully fetched %d records for %s", len(data), symbol
            )
            return data
            ...
            self.logger.error(
                "Unexpected error fetching Yahoo data for %s: %s", symbol, str(e)
            )
            return pd.DataFrame()
    ...
    def get_alpha_vantage_data(
        ...
        try:
            ...
            if "Time Series (Daily)" in data:
                ...
                self.logger.error(
                    "Unexpected Alpha Vantage response format for %s", symbol
                )
                return pd.DataFrame()
            ...
            self.logger.info(
                "Successfully fetched %d records for %s from Alpha Vantage",
                len(df),
                symbol,
            )
            return df
            ...
            self.logger.error(
                "Error fetching Alpha Vantage data for %s: %s", symbol, str(e)
            )
            return pd.DataFrame()
        except Exception as e:
            self.logger.error(
                "Unexpected error fetching Alpha Vantage data for %s: %s",
                symbol,
                str(e),
            )
            return pd.DataFrame()
    ...
    def get_stock_info(self, symbol: str) -> Dict:
        ...
        try:
            ...
            self.logger.error(
                "Unexpected error fetching stock info for %s: %s", symbol, str(e)
            )
            return {}
    ...
    def save_data(self, data: pd.DataFrame, filename: str, file_format: str = "csv"):
        ...
        try:
            ...
            self.logger.error(
                "Unexpected error saving data to %s: %s", filepath, str(e)
            )
    ...
    def load_data(self, filename: str, file_format: str = "csv") -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error(
                "Unexpected error loading data from %s: %s", filepath, str(e)
            )
            return pd.DataFrame()
