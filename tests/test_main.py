...
class TestTradingEngine(unittest.TestCase):
    ...
    def test_validate_strategy_parameters(self):
        ...
        is_valid = self.engine.validate_strategy_parameters('moving_average', valid_params)
        self.assertTrue(is_valid)
        
        # Invalid parameters - short window >= long window
        invalid_params = {'short_window': 30, 'long_window': 10}
        is_valid = self.engine.validate_strategy_parameters('moving_average', invalid_params)
...
class TestTradingEngineIntegration(unittest.TestCase):
    ...
    def test_real_backtest_integration(self):
        ...
        with patch.object(engine.data_fetcher, 'get_yahoo_data', return_value=sample_data), \
             patch.object(engine.data_fetcher, 'add_technical_indicators', return_value=sample_data):
...
class TestTradingEngineErrorHandling(unittest.TestCase):
    ...
    def test_backtest_with_data_fetch_error(self):
        ...
        with patch.object(self.engine, 'fetch_data', side_effect=Exception("Network error")):
    ...
    def test_invalid_symbol_handling(self):
        ...
        with patch.object(self.engine.data_fetcher, 'get_yahoo_data', return_value=pd.DataFrame()):
